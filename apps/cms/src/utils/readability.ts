...
const textRegex = /\s+/;
const sentenceRegex = /[.!?]+/;
const withoutERegex = /(?:[^laeiouy]es|ed|[^laeiouy]e)$/;
const withoutYRegex = /^y/;
...
function countSyllables(word: string): number {
  ...
  const lowerCaseWordWithoutEs = lowerCaseWord.replace(withoutERegex, "");
  const lowerCaseWordWithoutY = lowerCaseWordWithoutEs.replace(
    withoutYRegex,
    ""
  );

  const syllableMatches = lowerCaseWordWithoutY.match(syllableRegex);
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: this is stupid
export function generateSuggestions(metrics: {
  ...
  if (sentenceCount < metricsWordCount / 20) {
    suggestions.push(
      "Consider breaking up some longer sentences into shorter ones"
    );
  }

  if (metricsWordCount > 1000 && avgWordsPerSentence < 15) {
    suggestions.push(
