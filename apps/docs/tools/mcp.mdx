...
description: "Connect Claude, Cursor, and other Model Context Protocol clients to your Marble workspace to read, create, and manage content with AI agents."
---
...
The quickest way to add Marble to supported MCP clients is with the open-source
[`add-mcp` CLI](https://github.com/neon-solutions/add-mcp). It detects supported
...
npx add-mcp https://mcp.marblecms.com/mcp \
  --header 'Authorization: ${MCP_MARBLE_API_KEY}' \
  -g
```

Leave the placeholder in single quotes so your shell does not expand it before
`add-mcp` runs. Because the command is interactive, the CLI prompts for the
...
<Tabs>
  <Tab title="Cursor">
    <Card
      ...
      href="cursor://anysphere.cursor-deeplink/mcp/install?name=marble&config=eyJ0eXBlIjoic3RyZWFtYWJsZUh0dHAiLCJ1cmwiOiJodHRwczovL21jcC5tYXJibGVjbXMuY29tL21jcCIsImhlYWRlcnMiOnsiQXV0aG9yaXphdGlvbiI6IkJlYXJlciAke01DUF9NQVJCTEVfQVBJX0tFWX0ifX0="
      arrow={false}
    >
  ...
  <Tab title="VS Code">
    <Card
      ...
      href="vscode:mcp/install?%7B%22name%22%3A%22marble%22%2C%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fmcp.marblecms.com%2Fmcp%22%2C%22headers%22%3A%7B%22Authorization%22%3A%22%24%7BMCP_MARBLE_API_KEY%7D%22%7D%7D"
      arrow={false}
    >
...
The MCP server accepts your Marble API key through the standard API header:

```txt
Authorization: <your-api-key>
```

The `Bearer` prefix is also accepted, but is not required by Marble.
