...
<Steps>
  ...
  <Step title="Create a New Webhook">
    ...
    payload will be sent - **Format**: Choose JSON, Discord, or Slack -
...
Here's an example payload for a `post.updated` event:
...
The top-level envelope is the same for `post.published`, `post.deleted`, `tag.created`, `category.updated`, `author.deleted`, and other JSON events. The `type`, `resource`, and `data` fields change to match the event.
