...
description: "TypeScript types and Zod schemas for Marble API responses — Post, Author, Category, Tag, Pagination — via the SDK or copy-paste definitions."
---

<Tip>
  **We recommend using the [TypeScript SDK](/tools/sdk)** for the best
...
export type Author = {
  ...
  role: string | null;
  socials: Social[];
  // Present on list/detail endpoints, omitted when embedded in a post
  count?: {
    posts: number;
  };
};
