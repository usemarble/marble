---
title: "Marble API Rate Limits and Backoff"
description: "Marble API rate limits, X-RateLimit response headers, 429 error handling, and an exponential backoff pattern to keep clients within per-API-key quotas."
---
...
The Marble API uses rate limiting to ensure fair usage and API stability. Rate limits are enforced per API key using a sliding window approach.
...
<Tip>
  ...
  - **Batch operations**: Process items sequentially with delays rather than parallel requests.
  - **Monitor headers**: Track `X-RateLimit-Remaining` and slow down before hitting limits.
  - **Use webhooks**: For real-time updates, consider webhooks instead of polling the API.
</Tip>
